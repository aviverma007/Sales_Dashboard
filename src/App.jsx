                      ...
                      <ResponsiveContainer width="100%" height={210}>
                        <ComposedChart data={sl} margin={{top:26,right:8,bottom:18,left:0}} barGap={4} barCategoryGap="30%">
                          ...
                          <YAxis tick={{fill:T.textM,fontSize:9}} axisLine={false} tickLine={false} width={54} tickFormatter={v=>'₹'+v.toLocaleString('en-IN')}
                            domain={[()=>{const vals=sl.flatMap(d=>[d.achieved,d.target,d.projection]).filter(v=>v!=null&&v>0);return vals.length?Math.floor(Math.min(...vals)*0.97/500)*500:0;},()=>{const vals=sl.flatMap(d=>[d.achieved,d.target,d.projection]).filter(v=>v!=null&&v>0);return vals.length?Math.ceil(Math.max(...vals)*1.03/500)*500:30000;}]}
                          />
