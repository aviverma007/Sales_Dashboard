...
// ─── GLASS CARD ───────────────────────────────────────────────────────────────
// ─── ZOOM MODE CONTEXT ───────────────────────────────────────────────────────
// Double-click any card to zoom it in (centered, full detail, blurred
// background); double-click again (or click outside) to zoom back out.
// No on/off toggle needed - it's always available on every card.
const ZoomCtx = createContext({ pinnedId:null, setPinnedId:()=>{} });
let __gcIdSeq = 0;

const GC = ({children,style={},cls='',dark=false}) => {
  const [h,sH]=useState(false);
  const { pinnedId, setPinnedId } = useContext(ZoomCtx);
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          borderRadius:14,
          boxShadow: isPinned?'0 24px 70px rgba(0,20,50,0.45)':(dark?'0 8px 32px rgba(0,0,0,0.35)':'0 4px 24px rgba(0,80,120,0.12)'),
          transition:'all 0.2s ease', overflow:'hidden',
          cursor: isPinned?'zoom-out':'zoom-in',
          ...style,
          position:'relative',
          ...(isPinned ? {
        ...
        {h && (
          <div style={{
            ...
            fontSize:9, fontWeight:700, color: dark?'rgba(255,255,255,0.65)':'rgba(0,60,90,0.55)',
            background: dark?'rgba(0,0,0,0.35)':'rgba(255,255,255,0.85)',
            padding:'3px 8px', borderRadius:6, pointerEvents:'none',
            opacity:0.9,
          }}>
            {isPinned ? 'Double-click to zoom out' : 'Double-click to zoom'}
          </div>
        )}
        {children}
      </div>
...
const SummaryBar = ({raw, filters, T, GC}) => {
  ...
  let pdfnTotalUnits = booked.length; // default to INVR
...
function AppInner({overviewOnly=false, onBackToMenu, hideLogout}) {
  ...
  // Double-click any chart/KPI card to zoom it in (blurred background, centered,
  // full detail); double-click again, or click outside, to zoom back out.
  const [pinnedId, setPinnedId] = useState(null);
  // Click anywhere outside a pinned card closes it. The pinned card's own
  // double-click already toggles it off via handleDoubleClick/setPinnedId in GC,
  // and the blur overlay's onClick also unpins — this listener is a fallback for
  // any other outside click (e.g. header, filters) while a card is pinned.
  useEffect(() => {
...
const cnt={};(raw?.pdrn||[]).forEach(r=>{if(!selProjs.includes(r.project))return;if(r.status!=='ACTIVE')return;if(r.brokerName)cnt[r.brokerName]=(cnt[r.brokerName]||0)+1;});return Object.entries(cnt).sort((a,b)=>b[1]-a[1]).map(e=>e[0]);}const src=raw?.pdrn||[];const cnt={};src.forEach(r=>{if(r.status!=='ACTIVE')return;if(filters.company&&r.companyNorm!==filters.company)return;if(r.brokerName)cnt[r.brokerName]=(cnt[r.brokerName]||0)+1;});return Object.entries(cnt).sort((a,b)=>b[1]-a[1]).slice(0,50).map(e=>e[0]);},[raw,filters.project,filters.company]);
  ...
  const kpiEx=useMemo(()=>{
    ...
    const demandRaisedCr = pdrnDemandCr;
    const collectedCr    = pdrnReceivedCr;
    const outstandingCr  = pdrnOutstandingCr;
    return {
                ...
                <GC style={{padding:16}}>
                  ...
                  {(()=>{
                    ...
                    const rawDataR=monthlyWithTargets.map(d=>({label:d.label,isFuture:d.isFuture,isCurrent:d.label===TODAY_LABEL,
                      ...
                      requiredRate:(()=>{
                        const dym=lblYmR(d.label);
                        if(dym<todayYMR&&d.label!==TODAY_LABEL)return null; // only future months (keep current)
                        ...
                        {requiredRate>0&&(
                          <div style={{width:160,flexShrink:0,display:'flex',flexDirection:'column',gap:8}}>
                            ...
                            <div style={{background:'rgba(255,255,255,0.9)',border:'1.5px solid rgba(0,100,140,0.15)',borderRadius:8,padding:'8px 10px',flex:1}}>
                              ...
                              <p style={{fontSize:9,color:'#37474f',margin:0}}>required against ₹{currentMonthTargetRate.toLocaleString('en-IN')} (Target Rate) to maintain Total Project Value of ₹{aopTsvAtCurrentRate.toFixed(0)} Cr</p>
                            </div>
                          </div>
                        )}
                      </div>
