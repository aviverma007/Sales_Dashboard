                      ...
                      <ResponsiveContainer width="100%" height={210}>
                        <ComposedChart data={sl} margin={{top:26,right:8,bottom:18,left:0}} barGap={4} barCategoryGap="30%">
                          ...
                          <YAxis tick={{fill:T.textM,fontSize:9}} axisLine={false} tickLine={false} width={54} tickFormatter={v=>'₹'+v.toLocaleString('en-IN')}
                            domain={[()=>{const vals=sl.flatMap(d=>[d.achieved,d.target,d.projection]).filter(v=>v!=null&&v>0);if(!vals.length)return 0;const mn=Math.min(...vals);const mx=Math.max(...vals);const pad=Math.max((mx-mn)*0.5,500);return Math.floor((mn-pad)/250)*250;},()=>{const vals=sl.flatMap(d=>[d.achieved,d.target,d.projection]).filter(v=>v!=null&&v>0);if(!vals.length)return 30000;const mn=Math.min(...vals);const mx=Math.max(...vals);const pad=Math.max((mx-mn)*0.5,500);return Math.ceil((mx+pad)/250)*250;}]}
                          />
