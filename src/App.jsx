...
const cnt={};(raw?.pdrn||[]).forEach(r=>{if(!selProjs.includes(r.project))return;if(r.status!=='ACTIVE')return;if(r.brokerName)cnt[r.brokerName]=(cnt[r.brokerName]||0)+1;});return Object.entries(cnt).sort((a,b)=>b[1]-a[1]).map(e=>e[0]);}const src=raw?.pdrn||[];const cnt={};src.forEach(r=>{if(r.status!=='ACTIVE')return;if(filters.company&&r.companyNorm!==filters.company)return;if(r.brokerName)cnt[r.brokerName]=(cnt[r.brokerName]||0)+1;});return Object.entries(cnt).sort((a,b)=>b[1]-a[1]).slice(0,50).map(e=>e[0]);},[raw,filters.project,filters.company]);
  ...
  const kpiEx=useMemo(()=>{
    ...
    // Demand Raised sourced from PDRN 'Total Demand Amount' (active bookings) — but prefer kpiExtra.totalDemandWoTax if available
    const demandRaisedCr = dk.totalDemandWoTax || +(pAAll.reduce((s,r)=>s+(r.demand||0),0)/1e7).toFixed(1);
    // Collected = PDRN 'Total Received'; Outstanding = unpaid raised demand (floored per unit) — all active, w/o tax
    const collectedCr    = dk.totalReceivedWoT || +(pAAll.reduce((s,r)=>s+(r.received||0),0)/1e7).toFixed(1);
    const outstandingCr  = dk.totalOutstanding || +(pAAll.reduce((s,r)=>s+Math.max((r.demand||0)-(r.received||0),0),0)/1e7).toFixed(1);
    return {
