      ...
      <div style={{background:'rgba(255,255,255,0.97)',borderRadius:20,padding:'40px 44px',width:360,boxShadow:'0 24px 80px rgba(0,40,80,0.25)',backdropFilter:'blur(12px)'}}>
        ...
        <div style={{marginBottom:14}}>
          ...
          <input
            ...
            onFocus={e=>e.target.style.border='1.5px solid #0097a7'}
            onBlur={e=>e.target.style.border='1.5px solid rgba(0,100,140,0.2)'}
          />
        </div>
        <div style={{marginBottom:22}}>
          ...
          <div style={{position:'relative'}}>
            <input
              type={show?'text':'password'} value={p} onChange={e=>setP(e.target.value)}
              onKeyDown={e=>e.key==='Enter'&&submit()}
        ...
        <button onClick={submit} style={{width:'100%',padding:'12px',borderRadius:12,border:'none',background:'linear-gradient(135deg,#0097a7,#006978)',color:'#fff',fontSize:14,fontWeight:800,cursor:'pointer',letterSpacing:0.3,boxShadow:'0 6px 20px rgba(0,151,167,0.35)',transition:'opacity 0.15s'}} onMouseOver={e=>e.target.style.opacity=0.9} onMouseOut={e=>e.target.style.opacity=1}>
          Sign In →
        </button>
      ...
      <header style={{position:'sticky',top:0,zIndex:200,background:'rgba(255,255,255,0.95)',WebkitBackdropFilter:'blur(24px)',borderBottom:'1px solid rgba(255,255,255,0.9)',boxShadow:'0 2px 20px rgba(0,60,100,0.12)'}}>
        <div style={{maxWidth:1440,margin:'0 auto',padding:'0 24px',display:'flex',alignItems:'center',justifyContent:'space-between',height:54}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:14}}>
            ...
            <button onClick={()=>{sessionStorage.removeItem('sd_auth');setAuthed(false);}} style={{display:'flex',alignItems:'center',gap:6,padding:'6px 14px',borderRadius:10,border:'1px solid rgba(200,40,40,0.25)',background:'rgba(211,47,47,0.07)',cursor:'pointer',fontSize:11,fontWeight:700,color:T.red,fontFamily:'Inter,sans-serif',transition:'all 0.15s'}} onMouseOver={e=>{e.currentTarget.style.background='rgba(211,47,47,0.14)';}} onMouseOut={e=>{e.currentTarget.style.background='rgba(211,47,47,0.07)';}}>
              🔒 Logout
            </button>
          </div>
        </div>
